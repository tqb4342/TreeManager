...
function show(data){
	...
	//	console.log(data[list].code)
		var id = data[list].id;
			var tab = '<div class="flex-box col-table"><div class="row row-normal"><a href="lybk.html?wordname='+name+'" class="a-more-info" title="1">'+id+'</a></div>'+
...
function showKzblxdmbdata(){
	$(".table-body").empty();
	KzblxdmbManager.getLength(function(length){
		var n;
		if(length%20==0){
			n=length/20;
		}else{
	...
	//	$("#datapage").empty();
		...
		var pageId=1;
		KzblxdmbManager.getADataByPage(pageId,function(data){
			show(data);
		});
